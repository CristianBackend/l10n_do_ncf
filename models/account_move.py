...
B13_TRANSACTION_LIMIT = 10000  # RD$ por transacciÃƒÂ³n
...
class AccountMove(models.Model):
    ...
    l10n_do_report_period = fields.Char(
        string='PerÃƒÂ­odo Reportado',
        help='PerÃƒÂ­odo fiscal en que se reportÃƒÂ³ (YYYYMM)'
    )
    ...
    l10n_do_credit_note_reason = fields.Selection([
        ('01', '01 - AnulaciÃƒÂ³n total'),
        ('02', '02 - CorrecciÃƒÂ³n de errores'),
        ('03', '03 - DevoluciÃƒÂ³n de bienes'),
    ...
    # B03 Nota DÃƒÂ©bito
    l10n_do_is_debit_note = fields.Boolean(string='Es Nota de DÃƒÂ©bito', default=False)
    ...
    l10n_do_fiscal_type = fields.Selection([
        ...
        ('special', 'B14 - RÃƒÂ©gimen Especial'),
    ...
    l10n_do_informal_provider_cedula = fields.Char(string='CÃƒÂ©dula Proveedor')
    ...
    l10n_do_informal_service_type = fields.Selection([
        ...
        ('technical', 'Servicios TÃƒÂ©cnicos (ISR 2%)'),
        ('goods', 'Bienes (ISR 2%)'),
    ...
    l10n_do_payment_credit = fields.Monetary(
        string='CrÃƒÂ©dito', currency_field='currency_id', default=0.0
    )
    ...
    l10n_do_forma_pago = fields.Selection([
        ...
        ('04', '04 - CrÃƒÂ©dito'),
        ('05', '05 - Permuta'),
        ('06', '06 - Nota CrÃƒÂ©dito'),
        ('07', '07 - Mixto'),
    ], string='Forma Pago', compute='_compute_forma_pago', store=True)

    # =========================================
    # RETENCIÃƒâ€œN POSTERIOR (607)
    # =========================================

    l10n_do_is_credit_sale = fields.Boolean(string='Venta a CrÃƒÂ©dito', default=False)
    l10n_do_retention_date = fields.Date(string='Fecha RetenciÃƒÂ³n')
    ...
    l10n_do_retention_reported = fields.Boolean(string='RetenciÃƒÂ³n Reportada 607', default=False)

    l10n_do_needs_607_retention_line = fields.Boolean(
        string='Requiere LÃƒÂ­nea RetenciÃƒÂ³n 607',
        compute='_compute_needs_607_retention', store=True
    )

    # =========================================
    # CLASIFICACIÃƒâ€œN 606 - BIENES/SERVICIOS AUTO
    # =========================================
    ...
    l10n_do_expense_type = fields.Selection([
        ...
        ('05', '05 - Gastos RepresentaciÃƒÂ³n'),
        ('06', '06 - Otras Deducciones'),
        ('07', '07 - Gastos Financieros'),
        ('08', '08 - Gastos Extraordinarios'),
        ('09', '09 - Costo de Venta'),
        ('10', '10 - AdquisiciÃƒÂ³n Activos'),
    ...
    # NUEVO: Split automÃƒÂ¡tico bienes/servicios
    ...
    l10n_do_tipo_retencion_isr = fields.Selection([
        ...
        ('02', '02 - Honorarios'),
        ('03', '03 - Otras rentas'),
        ('04', '04 - PresunciÃƒÂ³n renta'),
        ('05', '05 - Intereses PJ'),
        ('06', '06 - Intereses PF'),
        ('07', '07 - Proveedores Estado'),
        ('08', '08 - Juegos telefÃƒÂ³nicos'),
    ], string='Tipo RetenciÃƒÂ³n ISR')
    ...
    # =========================================
    # CÃƒâ€œMPUTO SPLIT BIENES/SERVICIOS AUTOMÃƒÂTICO
    # =========================================
    ...
    def _compute_606_split_bienes_servicios(self):
        """
        Split automÃƒÂ¡tico basado en tipo de producto.
    ...
    def _compute_forma_pago(self):
        for move in self:
            ...
            if len(active_payments) > 1:
                ...
                move.l10n_do_forma_pago = '04'  # CrÃƒÂ©dito
    ...
    # =========================================
    # VALIDACIONES CÃƒâ€°DULA RD (NUEVO)
    # =========================================

    def _validate_cedula_rd(self, cedula):
        """
        Validar cÃƒÂ©dula dominicana con algoritmo Luhn modificado.
        """
        if not cedula:
            return False
    ...
    def _validate_rnc_rd(self, rnc):
        """
        Validar RNC dominicano con MÃƒÂ³dulo 11.
        """
        if not rnc:
            return False
    ...
    def _check_cedula_format(self):
        """Validar formato de cÃƒÂ©dula para B11"""
        for move in self:
            if move.l10n_do_fiscal_type == 'informal' and move.l10n_do_informal_provider_cedula:
                ...
                if not self._validate_cedula_rd(cedula):
                    raise ValidationError(_(
                        'CÃƒÂ©dula invÃƒÂ¡lida: %s\n\n'
                        'Verifique que:\n'
                        '- Tenga 11 dÃƒÂ­gitos\n'
                        '- El dÃƒÂ­gito verificador sea correcto'
                    ) % cedula)
        ...
        for move in self:
            if move.l10n_do_vendor_ncf and move.move_type in ('in_invoice', 'in_refund'):
                ...
                if not re.match(NCF_FULL_PATTERN, ncf):
                    raise ValidationError(_('NCF proveedor invÃƒÂ¡lido: %s') % ncf)
    ...
    def _check_dgii_reported_block(self):
        ...
        if self.l10n_do_reported_606 or self.l10n_do_reported_607:
            raise UserError(_(
                'Ã°Å¸Å¡Â« DOCUMENTO BLOQUEADO\n\n'
                'Este documento ya fue reportado a DGII en el perÃƒÂ­odo %s.\n\n'
                'No puede ser modificado ni cancelado.\n\n'
                'Para corregir errores debe:\n'
                '- Emitir Nota de CrÃƒÂ©dito (NC)\n'
                '- O realizar ajuste en perÃƒÂ­odo siguiente'
            ) % (self.l10n_do_report_period or 'anterior'))

    def button_cancel(self):
        """Override para bloquear cancelaciÃƒÂ³n post-reporte"""
        for move in self:
    ...
    def unlink(self):
        """Override para bloquear eliminaciÃƒÂ³n post-reporte"""
        for move in self:
            if move.l10n_do_reported_606 or move.l10n_do_reported_607:
                raise UserError(_(
                    'Ã°Å¸Å¡Â« No puede eliminar documentos reportados a DGII.\n'
                    'Documento: %s | PerÃƒÂ­odo: %s'
    ...
    def _check_informal_provider_rnc(self):
        ...
        if self.partner_id and self.partner_id.vat:
            ...
            if vat and len(vat) >= 9:
                raise UserError(_(
                    'Ã¢Å¡Â Ã¯Â¸Â NO puede usar B11 para proveedor con RNC.\n\n'
                    'Proveedor: %s\nRNC: %s\n\n'
        ...
        for line in self.invoice_line_ids:
            for tax in line.tax_ids:
                if tax.amount > 0 and 'itbis' in (tax.name or '').lower():
                    raise UserError(_(
                        'Ã¢Å¡Â Ã¯Â¸Â B13 (Gasto Menor) no permite ITBIS acreditable.\n\n'
                        'LÃƒÂ­nea: %s\nImpuesto: %s\n\n'
                        'Use impuesto 0%% o quite el impuesto ITBIS.'
                    ) % (line.name, tax.name))

        # Validar lÃƒÂ­mite de monto B13
        if self.amount_total > B13_TRANSACTION_LIMIT:
            raise UserError(_(
                'Ã¢Å¡Â Ã¯Â¸Â B13 excede lÃƒÂ­mite de RD$ %s por transacciÃƒÂ³n.\n\n'
                'Monto: RD$ %s\n\n'
    ...
    def _check_b17_no_itbis(self):
        ...
        if self.amount_tax > 0:
            raise UserError(_(
                'Ã¢Å¡Â Ã¯Â¸Â B17 (Pago Exterior) no debe tener ITBIS.\n\n'
                'Los pagos al exterior estÃƒÂ¡n exentos de ITBIS.\n'
                'Use impuesto 0%% y aplique retenciÃƒÂ³n ISR 27%%.'
            ))

        # Verificar ISR 27%
        has_isr_27 = any(ret.retention_type_id.code == 'ISR_EXT' for ret in self.l10n_do_retention_ids)
        if not has_isr_27:
            raise UserError(_('B17 requiere retenciÃƒÂ³n ISR 27%.'))
    ...
    def _generate_ncf(self):
        ...
        if sequence.current_number > sequence.final_number:
            ...
            if alt_sequence:
                ...
                raise UserError(_(
                    'Ã¢Å¡Â Ã¯Â¸Â SECUENCIA AGOTADA\n\n'
                    'La secuencia %s estÃƒÂ¡ agotada y no hay alternativas.\n\n'
        ...
        self.env['l10n_do_ncf.fiscal.audit'].log_event(
            'ncf_generated', move=self, description='NCF generado automÃƒÂ¡ticamente'
        )
    ...
    def _apply_b11_retentions(self):
        """Retenciones B11: 100% ITBIS + ISR segÃƒÂºn tipo servicio"""
        retentions = []
    ...
    def _onchange_partner_check_rnc_change(self):
        ...
        if self.partner_id.vat:
            ...
            if b02_invoices:
                return {
                    'warning': {
                        'title': _('Ã¢Å¡Â Ã¯Â¸Â Cliente con RNC nuevo'),
                        'message': _(
    ...
    def action_verify_informal_rnc(self):
        ...
        self.l10n_do_informal_rnc_verified = True
        return {'type': 'ir.actions.client', 'tag': 'display_notification',
                'params': {'title': _('Ã¢Å“â€œ Verificado'), 'message': _('Sin RNC'), 'type': 'success'}}

    def action_add_retention(self):
        return {
            'type': 'ir.actions.act_window', 'name': _('Agregar RetenciÃƒÂ³n'),
    ...
    def action_mark_reported_606(self):
        ...
        return {'type': 'ir.actions.client', 'tag': 'display_notification',
                'params': {'title': _('Ã¢Å“â€œ Marcado'), 'message': _('Reportado 606 perÃƒÂ­odo %s') % period, 'type': 'success'}}
    ...
    def action_mark_reported_607(self):
        ...
        return {'type': 'ir.actions.client', 'tag': 'display_notification',
                'params': {'title': _('Ã¢Å“â€œ Marcado'), 'message': _('Reportado 607 perÃƒÂ­odo %s') % period, 'type': 'success'}}

    def action_mark_retention_reported(self):
        self.ensure_one()
        self.l10n_do_retention_reported = True
        return {'type': 'ir.actions.client', 'tag': 'display_notification',
                'params': {'title': _('Ã¢Å“â€œ Marcada'), 'message': _('RetenciÃƒÂ³n reportada en 607.'), 'type': 'success'}}
    ...
    def _get_606_line_data(self):
        """Datos completos para lÃƒÂ­nea 606"""
        self.ensure_one()
    ...
    def _get_607_lines_data(self):
        """
        Datos para 607 incluyendo segunda lÃƒÂ­nea de retenciÃƒÂ³n posterior.
        """
        self.ensure_one()
        lines = []
        ...
        # Segunda lÃƒÂ­nea si hay retenciÃƒÂ³n posterior
